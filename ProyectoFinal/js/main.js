...
const loadFooter = () => {
    ...
    const popup = footer ? footer.querySelector(`.u-Background`) : null
...
const AppData = (() => {
    ...
    // Guarda información de navegación para el header
    ...
    // Establece país seleccionado y omite coger información de otros países
    ...
    // Permite añadir o eliminar tours al carrito
...
const updateHeader = () => {
    ...
    links.forEach(({ id, file, unlocked }) => {
        ...
        link.classList.toggle(`isActive`, page === file)
...
const loadPartial = async (selector, file, hookName) => {
    ...
    if (!element) return
    element.innerHTML = await (await fetch(file)).text()
    window[hookName]?.()
}
