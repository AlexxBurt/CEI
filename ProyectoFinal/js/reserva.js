...
formButton.addEventListener(`click`, () => {
  if (!formButton.classList.contains(`isVisible`)) return
...
// El botón de Finalizar Compra abre el popup de pago
const checkoutButton = document.querySelector(`.Checkout-button`)
const paymentContainer = document.querySelector(`.Payment`)
const paymentBackground = paymentContainer ? paymentContainer.closest(`.u-Background`) : null

if (checkoutButton && paymentBackground) {
  checkoutButton.addEventListener(`click`, () => {
    paymentBackground.classList.add(`isVisible`)
  })
}
...
// El botón de Cancelar y el svg cierran el popup de pago
const paymentCancel = document.querySelector(`.Payment-cancel`)
const paymentCloseIcon = document.querySelector(`.Payment-svg`)

const closePaymentPopup = () => {
  if (!paymentBackground) return
  paymentBackground.classList.remove(`isVisible`)
}

if (paymentCancel) paymentCancel.addEventListener(`click`, closePaymentPopup)
if (paymentCloseIcon) paymentCloseIcon.addEventListener(`click`, closePaymentPopup)
...
const renderCheckoutByCountry = async () => {
  ...
  const paint = () => {
    ...
    tourRows.forEach((row, i) => {
      ...
      if (!t) return
      tourNames[i].textContent = t.name
